...
public class MuscleZombie : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        AtkSound = Resources.Load<AudioClip>("giantAtk");
        AggroSound = Resources.Load<AudioClip>("giantAggro");
        ...
        void Suspect()
    {
        ...
        if(detectionTime >= 1f || detectionTime02 >= 6f && detectionTime >= 2f || detectionTime02 >= 12f)
        {
            ...
            if(!hasAggro)
            {
                audioSrc.PlayOneShot(AggroSound);
                hasAggro = true;
            }
            sign1.SetActive(false);
        }
